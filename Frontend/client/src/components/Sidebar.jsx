...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  const { chats, selectedChat,setSelectedChat, theme, setTheme, user, navigate } =
    useAppContenxt();
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-scroll mt-3 text-sm space-y-3">
        {chats
          .filter((chat) =>
            (chat.messages[0] ? chat.messages[0].content : chat.name)
              .toLowerCase()
      ...
      <div
        ...
        className="flex items-center gap-2 mt-4 0 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all"
      >
        <img
      ...
      <div className="flex items-center justify-between gap-2 mt-4  dark:border-white/15 rounded-md ">
        <div className="flex flex-col text-sm">
