...
const ChatBox = () => {
  const {selectedChat,theme}= useAppContenxt()
    ...
    <div className="flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40">
      ...
      {mode == 'image' && (
        <label className='inline-flex items-center gap-2 mb-3 text-sm mx-auto'>
          ...
          <input type="checkbox"  className='cursor-poin' checked={isPublished} onChange={(e)=>setIsPublished(e.target.checked)}/>
        </label>
      )}
      ...
      <form onSubmit={onSubmit} className='bg-primary/20 dark:bg-[#583C79]/30 border border-primary dark:border-[#80609F]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex gap-4 items-center'>
       ...
       <input onChange={(e)=>setPrompt(e.target.value)} type="text" className='flex-1 w-full text-sm outline-none' value={prompt} required/>
