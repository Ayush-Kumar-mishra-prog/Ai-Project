...
const Community = () => {
  ...
  return (
    <div className="p-6 pt-12 xl:px-12 2xl:px-20 w-full max-auto h-full overflow-y-scroll">
   ...
   {images.map((item,index)=>(
    <a key={index} href={item.imageUrl} target="_blank" className="relative group block rounded-lg overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md transition-shadow duration-300">
      ...
      <p className="absolute bottom-0 right-0 text-xs bg-black/50 backdrop-blur text-white px-4 py-1 rounded-t-xl opacity-0 group-hover:opacity-100 transition duration-300">Created by {item.userName}</p>
    </a>
   ))}
      </div>
    ):<p className="text-center text-gray-600 dark:text-purple-200 mt-10">
      No images avalilable
      </p>}
  </div>
  )
};
