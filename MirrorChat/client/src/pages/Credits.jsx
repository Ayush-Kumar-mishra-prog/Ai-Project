...
const Credits = () => {
  ...
  const { axios, token } = useAppContenxt();
  const fetchPlans = async () => {
    try {
      const { data } = await axios.get("/api/payment/getPlans", {
  ...
  return (
    <div className="max-w-7xl  h-screen over-flow-y-scroll mx-auto px-4 sm:px-6 lg:px-8 py-12 ">
        ...
        {plans.map((plan) => (
          ...
          >
            ...
            <button
              ...
              className="mt-6 bg-purple-600 hover:bg-purple-700 active:bg-purple-800 text-white font-medium py-2 runded transition-colors cursor-pointer"
            >
              Buy Now
            </button>
          </div>
